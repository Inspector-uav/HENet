...
class NonBottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        ch1 = F.avg_pool2d(x, kernel_size=(1, feature_size))
        output1 = self.conv3x1_1(ch1)
        output1 = F.interpolate(output1, size=(x.size(2), x.size(3)), mode='bilinear', align_corners=True)
        para1 = output1.softmax(-2)

        ch2 = F.avg_pool2d(x, kernel_size=(feature_size, 1))
        output2 = self.conv1x3_1(ch2)
        output2 = F.interpolate(output2, size=(x.size(2), x.size(3)), mode='bilinear', align_corners=True)
        para2 = output2.softmax(-1)

        output = para1 * output1 + para2 * output2 + x
