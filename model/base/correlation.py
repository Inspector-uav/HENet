    ...
    def multilayer_correlation(cls, query_feats, support_feats, stack_ids):
        ...
        for idx, (query_feat, support_feat) in enumerate(zip(query_feats, support_feats)):
            ...
            support_feat = support_feat / (
                        support_feat.norm(dim=1, p=2, keepdim=True) + eps)  # torch.Size([1, 512, 2500])
            ...
            corr = torch.bmm(query_feat.transpose(1, 2), support_feat).view(bsz, ha, wa, hb,
                                                                            wb)  # torch.Size([1, 50, 50, 50, 50])
